...
class MainScreen(MDScreen):
    ...
    def build_ui(self) -> None:
        ...
        header_text = "🤖 aiDroid für Samsung S25" if self.is_samsung_s25 else "🤖 aiDroid"
        title = MDLabel(
            text=header_text,
        ...
        self.text_input = MDTextField(
            ...
            size_hint_y=None,
            height="120dp",
            helper_text="Optimiert für One UI 7" if self.is_samsung_s25 else "KI-Chat Interface",
        ...
        send_button = MDRaisedButton(
            text="📤 Senden",
            size_hint_x=0.7,
            on_release=self.send_message_async
        )
    ...
    def send_message_async(self, instance: Any) -> None:
        ...
        # Async verarbeiten
        Clock.schedule_once(lambda dt: asyncio.create_task(self._process_message_async(message)), 0)
    ...
    async def _process_message_async(self, message: str) -> None:
        ...
        try:
            ...
            if self.network_handler:
                ...
                Clock.schedule_once(lambda dt: self.update_response(f"{prefix} {response}"), 0)
            else:
                Clock.schedule_once(lambda dt: self.update_response("❌ Network handler nicht verfügbar"), 0)
    ...
    def clear_history(self, instance: Any) -> None:
        """History löschen"""
        self.response_label.text = "🗑️ Historie gelöscht\n💬 Bereit für neue Nachrichten..."
        self.message_count = 0
        gc.collect()
    ...
    def force_gc(self, instance: Any) -> None:
        ...
        gc.collect()
        self.status_label.text = f"🧹 Speicher bereinigt (Nachrichten: {self.message_count})"

    def reset_status(self) -> None:
        """Status zurücksetzen"""
        status_text = "✅ Bereit für Samsung Galaxy S25" if self.is_samsung_s25 else "✅ Bereit"
